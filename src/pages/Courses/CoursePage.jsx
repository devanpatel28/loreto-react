        ...
        <div className="max-w-full overflow-x-auto">
          ...
          <thead>              
            <tr className="bg-gray-2 text-left dark:bg-meta-4">
              ...
              <th onClick={() => sorting("course_name")} className="min-w-[200px] py-4 px-4 font-medium text-black dark:text-white xl:pl-11">
                <span className="flex items-center gap-1">
                  Course Name
                  {order === 'ASC' ? <FcAlphabeticalSortingZa/>  : <FcAlphabeticalSortingAz/>}
                </span>
              </th>
              <th onClick={() => sorting("semester")} className="min-w-[240px] py-4 px-4 font-medium text-black dark:text-white">
                <span className="flex items-center gap-1">
                  Semester
                  {order === 'ASC' ? <FcAlphabeticalSortingZa/>  : <FcAlphabeticalSortingAz/>}
                </span>
              </th>
              <th onClick={() => sorting("status")} className="min-w-[120px] py-4 px-4 font-medium text-black dark:text-white">
                <span className="flex items-center gap-1">
                  Status
                  {order === 'ASC' ? <FcAlphabeticalSortingZa/>  : <FcAlphabeticalSortingAz/>}
                </span>
            ...
            <tbody>
              {currentItems.filter((item) => {
                return search.toLowerCase() === '' ? item : item.course_name.toLowerCase().includes(search) || item.semester.toLowerCase().includes(search)}).map((packageItem, key) => (
                <tr key={key}>
